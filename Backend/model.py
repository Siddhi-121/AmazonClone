...
class UserAddress(BaseModel):
    ...
    country = models.CharField(
        verbose_name="Country", choices=(("India", "India"),), max_length=100
    )
    full_name = models.CharField(
        verbose_name="Full name (First and last name)", max_length=100
    )
    ...
    flat = models.CharField(
        verbose_name="Flat, House no., Building, Company, Apartment", max_length=200
    )
    street = models.CharField(
        verbose_name="Area, Street, Sector, Village", max_length=200
    )
    ...
    state = models.CharField(
        verbose_name="State", choices=STATE_CHOICES, max_length=100
    )
    default = models.BooleanField(
        verbose_name="Make this my default address", default=False
    )
...
class Product(BaseModel):
    ...
    brand = models.ForeignKey(
        Brand, on_delete=models.SET_NULL, related_name="Brand_Name", null=True
    )
    category = models.ForeignKey(
        Category, on_delete=models.SET_NULL, related_name="Category", null=True
    )
    subcategory1 = models.ForeignKey(
        Subcategory, on_delete=models.SET_NULL, related_name="SubCategory1", null=True
    )
    subcategory2 = models.ForeignKey(
        Subcategory, on_delete=models.SET_NULL, related_name="SubCategory2", null=True
    )
...
class ProductDetail(BaseModel):
    ...
    color = models.ForeignKey(
        Color, on_delete=models.SET_NULL, null=True, related_name="Color"
    )
    ...
    discount = models.DecimalField(
        verbose_name="Discount", decimal_places=2, max_digits=10
    )
...
class Cart(BaseModel):
    user = models.ForeignKey(
        Amazonuser, on_delete=models.CASCADE, related_name="AmazonUser"
    )
    product = models.ForeignKey(
